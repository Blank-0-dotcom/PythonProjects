...
# lets create a board object to represent them inesweeper game
...
class Board:
    ...
    def make_new_board(self):
        ...
        #  [...                  ],
        #  [None, None, ..., None]]
        # we can see how this represents a board! (dim_size x dim_size)

        # plant the bombs
        bombs_planted = 0
        while bombs_planted < self.num_bombs:
            loc = random.randint(0, self.dim_size**2 - 1)  # **2 for square
            # return a random integer N such that a <= N <= b
            # // divide with integral result (discard remainder)
            row = loc // self.dim_size  # no. of times dim_size goes into loc
            col = loc % self.dim_size  # remainder to tell us what index in that row to look at

            if board[row][col] == "*":
                # this means we've actually planted a bomb there already so keep going
                continue  # find another location
            board[row][col] = "*"  # if there's no bomb, plant 1
            bombs_planted += 1
        return board
    ...
    def assign_values_to_board(self):
        ...
        for r in range(self.dim_size):
            for c in range(self.dim_size):
                if self.board[r][c] == "*":
                    # if this is already a bomb, we don't want to calculate anything/ overwrite our bombs planted
                    continue
    ...
    def dig(self, row, col):
        ...
        # dig at lication with no neighbouring bombs -> recursively dig neighbours
    ...
    def __str__(self):
        ...
        # it will print our what this function returns!
...
def play(dim_size=10, num_bombs=10):
    ...
    # Step 2: show the board to the user and ask where they want to dig
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        ...
        # 0,0 or 0, 0, or 0,     0
        user_input = re.split(
            ",(\\s)*", input("Where whould you like to dig? Input as (row, col): "))
